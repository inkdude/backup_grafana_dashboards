...
do
    ...
    do
        ...
        curl -sS -k -H "Authorization: Bearer $KEY" $URL/api/dashboards/$d > $DASH_DIR/$f/$normal_name.json
    done
done
...
do
        ...
        curl -sS -k -H "Authorization: Bearer $KEY" $URL/api/dashboards/$wf > $DASH_DIR/$normal_name.json
done
